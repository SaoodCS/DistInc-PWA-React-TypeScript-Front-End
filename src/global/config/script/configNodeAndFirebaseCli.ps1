if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
    ...
    Write-Host 'chocolatey has been installed. PLEASE RESTART THE CMD OR CODE EDITOR and run this script again.'
    exit
}
if (-not (Get-Command nvm -ErrorAction SilentlyContinue)) {
    Write-Host 'nvm is not installed. Installing nvm using choco...'
    choco install nvm -y
    Write-Host 'nvm has been installed. PLEASE RESTART THE CMD OR CODE EDITOR and run this script again.'
    exit
}
...
Write-Host 'Env setup is complete'
